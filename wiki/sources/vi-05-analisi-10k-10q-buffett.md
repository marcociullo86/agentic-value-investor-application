---
type: source
sources: ["raw/05_Analisi_10K_10Q_e_Regole_Buffett.md"]
status: draft
created: 2026-05-20
updated: 2026-05-20
tags: [value-investing, buffett, 10-k, 10-q, sec, economic-moat, roic, roe, dcf, margin-of-safety, vi-domain]
domain: value-investing
---
# Source: Analisi 10-K/10-Q e Metodo Warren Buffett

> Guida operativa alla lettura dei report SEC (10-K annuale, 10-Q trimestrale) con i filtri quantitativi e qualitativi del metodo Buffett.

## Contesto

Raw che fornisce un runbook in 5 step per sezionare il 10-K/10-Q, e codifica le regole quantitative Buffett (ROE >15%, ROIC >12-15%, margini, debito, CapEx) e i quattro tipi di moat. [^src: raw/05_Analisi_10K_10Q_e_Regole_Buffett.md §1. Introduzione: 10-K vs 10-Q]

## Sezioni mappate

| Sezione raw | Pagina wiki |
|---|---|
| §1 10-K vs 10-Q | [[sec-filings-analysis]] |
| §2 Step procedurali (Item 1, 1A, 7, 8, Note) | [[sec-filings-analysis]], [[sec-10k-10q-analysis-playbook]] |
| §3A Cerchio di Competenza | [[warren-buffett]] |
| §3B Fossato Economico (4 forme) | [[economic-moat]] |
| §3C Regole finanziarie quantitative | [[intrinsic-value]], [[fmp-key-metrics-ratios]] |
| §3D Valutazione del Management | [[warren-buffett]] |
| §4 Margine di Sicurezza (Owner Earnings, DCF) | [[margin-of-safety]], [[intrinsic-value]] |

## Concetti correlati
[[sec-filings-analysis]]
[[economic-moat]]
[[margin-of-safety]]
[[intrinsic-value]]
[[warren-buffett]]
[[fmp-financial-statements-stable]]
[[fmp-key-metrics-ratios]]

## Pagine collegate
[[vi-03-analisi-fondamentale-valutazione]]
[[vi-04-gestione-rischio-psicologia]]

## Storie collegate
<!-- Sezione gestita dal product-manager — non modificare se sei wiki-keeper -->
