...
## [2026-05-22] ingest | fmp_docs.md + fmp_docs.json (migrazione FMP v3 -> stable)
Cancellate: 21 pagine wiki FMP v3 obsolete (8 sources, 9 concepts, 1 runbook, 1 entity, 2 syntheses) — rimpiazzate da tombstone con redirect. Sorgenti raw originali (FMP_Docs_1..8.txt) erano gia' stati eliminati dall'utente. Nuove fonti di verita': `raw/fmp_docs.md` (narrativo, ~500 KB) + `raw/fmp_docs.json` (strutturato, 263 record endpoint `/stable`).
Pagine create: 17 (1 source, 13 concepts, 1 entity, 2 syntheses, 1 runbook) | Aggiornamenti: 3 (index, log, gaps) | Gap chiusi: 1 (fmp-endpoint-base-urls — risolto con URL verified da raw) | Gap parzialmente chiusi: 2 (fmp-rate-limiting, fmp-error-codes — nuovi raw non li risolvono completamente) | Gap nuovi aperti: 3 (fmp-stable-rate-limiting, fmp-stable-analyst-estimates, fmp-stable-adapter-migration)
