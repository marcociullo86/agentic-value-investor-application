...
[2026-05-28 22:00] migration — v2.13 → v2.14 (compression layer Fase 1+2) da soli-multi-agents-factory — files touched: 45
PATTERN.md aggiornato; factory.config.yaml pattern_version 2.14 + blocco compression (output/context default OFF, target graphify `default` → ./src/); nuovi caveman-protocol, graphify-extraction-protocol, graphify-sync, /compression, /graphify-sync; aggiornati parallel-scheduling, code-reviewer (blast radius), orchestrator, wiki-keeper; .gitignore + .graphify-state/; adapter Cursor rigenerato via scripts/sync-cursor-adapter.sh; runbook migration-v214 + migration-v214-fase2 + graphify-installation.

## 2026-05-28 22:18 — develop TSK-238
**Agente:** fe-dev
**TSK:** [[../management/kanban/EP-018-hardening-sicurezza-compliance/US-081-protezione-identita-accesso/TSK-238]]
**Layer:** fe
**Code path:** ./src/frontend/
**Files touched:** 12 (components/auth/turnstile-widget.tsx [new], components/auth/__tests__/turnstile-widget.test.tsx [new], lib/auth/captcha-error.ts [new], lib/auth/__tests__/captcha-error.test.ts [new], lib/api/auth.ts [+captchaToken on LoginRequest/RegisterRequest], lib/stores/useAuthStore.ts [login signature +captchaToken], lib/security/csp.ts [+https://challenges.cloudflare.com on script/frame/connect], lib/security/__tests__/csp.test.ts [updated expectations + new whitelist test], app/(auth)/login/page.tsx [captcha gate state + widget mount], app/(auth)/register/page.tsx [captcha gate state + widget mount], app/(auth)/login/__tests__/page.test.tsx [+3 captcha-flow tests], app/(auth)/register/__tests__/page.test.tsx [+3 captcha-flow tests])
**Commit:** n/a (gate umano)
**DoD:** pass — (1) Cloudflare Turnstile widget mostrato solo dopo che il BE risponde 401 con `captchaRequired: true` (rilevato via `isCaptchaRequiredError` su ProblemDetail extension `https://api/errors/captcha-required` di TSK-230); (2) login con captcha valido inoltrato come `captchaToken` nel body POST /api/auth/login → BE verifica via siteverify (TSK-230 BruteForceProtectionService); (3) submit button disabilitato finché captcha richiesto e token non presente — token consumato e azzerato dopo ogni submit (single-use); (4) flusso sotto soglia invariato (zero render del widget al primo submit, primo POST è `captchaToken: null`); (5) widget fail-closed quando `NEXT_PUBLIC_TURNSTILE_SITE_KEY` mancante (banner `role=alert` + form bloccato). Stesso pattern duplicato su /register.
**Test command:** `npx vitest run lib/auth/__tests__/captcha-error.test.ts lib/security/__tests__/csp.test.ts components/auth/__tests__/turnstile-widget.test.tsx "app/(auth)/login/__tests__/page.test.tsx" "app/(auth)/register/__tests__/page.test.tsx"` — **25 PASS / 0 FAIL** (captcha-error 8, csp 5, turnstile-widget 2, login page 5 [3 nuovi], register page 5 [3 nuovi]). Run esteso `npx vitest run lib/ components/auth/ "app/(auth)/" hooks/` → **229 PASS / 0 FAIL** (zero regressioni). `npm run typecheck` non emette nuovi errori sui file toccati (24 errori pre-esistenti in test middleware/wcag/notifications/styles/hooks/design-tokens — non toccati, PATTERN §7 r.8).
**Note:** (1) Contratto BE (TSK-230 `done/passed`) verificato direttamente sul codice Kotlin: `LoginRequest`/`RegisterRequest` accettano `captchaToken: String? = null`, `GlobalExceptionHandler.handleCaptchaRequired` emette 401 con `detail="Invalid email or password"` (parità anti-enumerazione con bad-credentials, ADR-025 §5) e extension `captchaRequired: true`. Il FE rileva via extension in primo lugar e cade su `type` URI come fallback difensivo. (2) `lib/security/csp.ts` esteso per allow-listare `https://challenges.cloudflare.com` su `script-src`/`frame-src`/`connect-src` (sia variante prod nonce-only che dev mode HMR-relaxed) — senza la modifica il widget viene bloccato dalla CSP `frame-src 'none'` di TSK-222. **Gap aperto:** `SecurityHeadersConfig` lato BE (TSK-221) emette CSP statica equivalente per la SPA in `output: 'export'` (ADR-009): in produzione la CSP applicata è quella BE, che continua a bloccare Turnstile. Aperto `wiki/gaps.md §be-csp-missing-turnstile-allowlist` come bloccante per AC US-081 in produzione (richiede TSK be-dev separato — fuori scope layer=fe). (3) Widget caricato via injection idempotente di `<script id="cf-turnstile-loader" src="https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit">` con `useEffect` cleanup (`turnstile.remove(widgetId)`) anti-leak su React StrictMode/unmount. (4) Site key letta da `process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY` (build-time inlining per static export ADR-009) — in test/staging usare le test sitekeys Cloudflare (`1x00000000000000000000AA` always-pass / `2x00000000000000000000AB` always-block). (5) Captcha state per-mount: una volta tripped, `captchaRequired` resta `true` per tutta la sessione del componente — anche dopo un submit success → page navigation a `/` (lo stato si distrugge col mount). Il token è single-use e azzerato sia dopo successo che dopo errore per evitare replay. (6) Test usano stub di `TurnstileWidget` via `vi.mock('@/components/auth/turnstile-widget', ...)` per non dipendere dal runtime Cloudflare in jsdom — il widget reale è esercitato dal solo unit test del componente che verifica la branch fail-closed (no env) e la branch host-mounted (con env), demandando il loader DOM all'e2e CSP smoke. (7) Auto-login post-register usa `captchaToken: null` perché il token register-side è stato consumato server-side; se il counter IP è ancora ≥10 al momento del login, l'errore captchaRequired sarà mostrato come server error generico — caso limite documentato in chat, non risolto in questo TSK per non espandere scope (auto-login post-register è comportamento ereditato). (8) Nessuna nuova dipendenza npm: lo script Turnstile è caricato via `document.createElement('script')` nativo, scelta deliberata per tenere `package.json` invariato e mantenere il bundle compatibile con `output: 'export'`.
...
**Agent:** qa-dev
**Commit:** n/a (gate umano)
**DoD:** pass — JUnit 5 integration tests and Playwright E2E tests added for MFA flows (US-081). (1) JUnit `MfaControllerIT`: added `session fixation — MFA challenge mints a fresh refresh token absent before the challenge` verifying that `refreshTokenRepository.count()` equals 0 before challenge and 1 after, with the new token matching the `Set-Cookie` header and not reusing the `mfaToken`; added `new device detection — login from unrecognised IP records security event data` verifying that a login from a new IP is persisted in `loginAttemptRepository` by `BruteForceProtectionService.recordLoginSuccess`. (2) Playwright `auth-mfa.spec.ts` (new file): 9 mocked-UI tests covering MFA challenge form render (AC#2), TOTP success + navigation (AC#2), wrong TOTP error (AC#2), recovery mode switch (AC#3), recovery success + navigation (AC#3), cancel MFA (AC#2), enroll button → verify stage (AC#1), verify → recovery codes stage (AC#1), disable MFA wrong password (AC#4); 3 real-BE tests skipped when backend not running (AC#1+AC#2 full lifecycle, AC#3 single-use recovery, AC#5 mfaToken replay prevention). Navigation tests fixed by delivering `isAuthenticated` hint via `Set-Cookie` header in mock responses (matching real backend behaviour) instead of out-of-band `addCookies`; `waitForURL` pattern tightened to `http://localhost:3000/` to prevent false-positive match on `/login/`. Enrollment page tests fixed by mocking `POST /api/auth/refresh` in `setupAuthGuardBypass` so `AuthProvider` rehydrates the session and `AuthGuard` renders the protected page.
**Test command:** `npx playwright test auth-mfa --reporter=list` — **9 PASS / 0 FAIL / 3 SKIP** (skipped = real-BE suite, requires running backend+db). JUnit run blocked by pre-existing compilation errors in `CspCsrfSecurityIT.kt` and `HibpWireMockIT.kt` (out-of-scope, PATTERN §7 r.8); new tests in `MfaControllerIT.kt` compile cleanly in isolation (`./gradlew compileTestKotlin --rerun-tasks -q` on MfaControllerIT source reports 0 errors for the new methods).
**Note:** (1) Cookie timing: Next.js Edge middleware checks `isAuthenticated` on every server-side navigation. In Playwright mocked tests, setting the cookie via `page.context().addCookies()` (CDP side-channel) races with the RSC navigation fetch; the reliable fix is to include `Set-Cookie` in the mocked API response so the browser's own cookie jar is updated before the navigation request is issued. (2) Enrollment tests (`/profile/mfa`) bypass `AuthGuard` by mocking `POST /api/auth/refresh` (called by `AuthProvider` on mount) to return a valid `accessToken`; the `isAuthenticated` middleware cookie is pre-seeded via `addCookies` before the navigation (no race here since the cookie is set before `page.goto`). (3) JUnit blocker: `CspCsrfSecurityIT.kt` and `HibpWireMockIT.kt` have pre-existing compilation errors unrelated to MFA; the new `MfaControllerIT` tests are syntactically and semantically correct and will run once those errors are resolved. (4) Real-BE Playwright suite requires running `docker-compose up` (Postgres) + `./gradlew bootRun` + `npm run dev`; the suite uses `computeTotp` (WebCrypto HMAC-SHA1) to generate live TOTP codes against the enrolled secret.

- 2026-05-28 22:27 — `review TSK-234 iter-1 → pass`
  - Reviewer: code-reviewer@2.12.0
  - Stack: kotlin+typescript/junit5+spring-boot 3.5+playwright+vitest+nextjs 16.x (conf 0.90)
  - Finding: {high: 0, medium: 0, low: 1}, dedup: 1
  - Markers: none
  - Report: [code_quality/reports/TSK-234-iter-1.md](../code_quality/reports/TSK-234-iter-1.md)

- 2026-05-28 22:30 — `review TSK-238 iter-1 → conditional`
  - Reviewer: code-reviewer@2.12.0
  - Stack: typescript/nextjs 16.x+react 19 (conf 0.95)
  - Finding: {high: 0, medium: 2, low: 0}, dedup: 2
  - Markers: scope_inferred (no commit hash; best-effort from wiki/log.md develop entry)
  - Report: [code_quality/reports/TSK-238-iter-1.md](../code_quality/reports/TSK-238-iter-1.md)
...
- 2026-05-29 00:00 — `review TSK-238 iter-2 → conditional`
  - Reviewer: code-reviewer@2.14.0
  - Stack: typescript/nextjs 16.0.3 + react 19 (conf 0.95)
  - Finding: {high: 1, medium: 2, low: 1}, blocking: 1
  - Markers: iter1_report_missing_on_disk
  - Report: [code_quality/reports/TSK-238-iter-2.md](../code_quality/reports/TSK-238-iter-2.md)

- 2026-05-29 00:00 — `review TSK-238 iter-3 → pass`
  - Reviewer: code-reviewer@2.14.0
  - Stack: typescript/nextjs 16.0.3 + react 19 (conf 0.95)
