...
- **OpenAPI cleanup** (TSK-210): rimosso `refreshToken` da response schema, documentato Set-Cookie header; contract test `AuthOpenApiSchemaContractTest` (tag `@contract`, CI green post-fix lifecycle Testcontainers `0050a11`). [^src: management/kanban/EP-017-protezione-rotte-sessione/US-075-migrazione-storage-credenziali/TSK-210.md] [^src: src/backend/src/test/kotlin/com/valueinvesting/webapp/api/AuthOpenApiSchemaContractTest.kt]
