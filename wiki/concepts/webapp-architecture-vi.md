...
**News classification — pipeline a imbuto value-investing (2026-05-30).** Ridisegnato `NewsSentimentService`: il vecchio "classifica-ogni-news + voto di maggioranza" annegava un singolo segnale strutturale reale nel rumore di N notizie neutre (rischioso, dato che la classe dominante guida il veto `BOCCIATO_VALUE_TRAP` e il segnale `APPROVATO_PANIC_BUY`). Ora: (1) pre-filtro deterministico (dedup + scarto rumore Mr. Market + ranking per materialità/recency, top 12), (2) **una** sintesi LLM olistica (non N chiamate), (3) dominante **asimmetrica** — un solo `STRUCTURAL_DAMAGE` credibile vince sul conteggio (capital preservation). Finestra 90gg con `from`/`to`/`limit` espliciti. Persistenza robusta: `news_id` allargato a `VARCHAR(512)` (**V029**) perché l'URL FMP (usato come chiave, manca un id stabile) sforava i 200 char; sintesi con `maxTokens` ampio + estrazione JSON tollerante. Dettaglio: [[fmp-news-media]] §Uso nel progetto. [^src: src/backend/src/main/kotlin/com/valueinvesting/webapp/service/NewsSentimentService.kt] [^src: src/backend/src/main/resources/db/migration/V029__news_classification_widen_news_id.sql]
