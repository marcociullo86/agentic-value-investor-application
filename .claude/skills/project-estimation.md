...
Tool orchestrati (deterministici, `.claude/tools/analytics/`, no MCP):
...
```
bash .claude/tools/analytics/build-reference-class.sh \
...
```
bash .claude/tools/analytics/run-pert.sh --scope '<voci con O/M/P>' --unit days --output json
```
...
```
python .claude/tools/analytics/run-monte-carlo.py \
...
```
bash .claude/tools/analytics/estimate-project.sh --payload '<aggregated>' --output json
```
