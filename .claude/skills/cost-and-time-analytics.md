...
Tool orchestrati (Bash standalone in `.claude/tools/analytics/`, ADR-023 §A, no MCP):
...
```
bash .claude/tools/analytics/analyze-timeline.sh \
...
- **STOP su event store vuoto** — se il filtro produce 0 eventi, fermarsi con messaggio
  ...
  > (b) verificare che `record_task_event` sia attivo (US-033 / `.claude/tools/analytics/record-event.sh`).
...
```
bash .claude/tools/analytics/compute-agentic-cost.sh \
     --events '<agent-events>' --pricing analytics/pricing.yaml --output json
bash .claude/tools/analytics/compute-human-cost.sh \
...
```
bash .claude/tools/analytics/generate-report.sh \
